                                 ...
                                 m_settings{settings}
{
	...
	if(is_new)
	{
        ...
        for(const auto& user: m_pending_assignees)
        {
          ...
          const auto info = m_db.resolve_user(user);
          chip->addNew<Wt::WText>(info.display_name, Wt::TextFormat::Plain)
            ->setStyleClass(info.deleted ? "user-deleted" : "");
...
void task_editor_form_widget::rebuild_history()
{
	...
	for(const auto& ev: events)
	{
		...
		const auto actor_info = m_db.resolve_user(ev.actor);
		auto*      hdr        = entry->addNew<Wt::WText>(
      fmt_ts(ev.occurred_at) + "  \xe2\x80\x94  " + actor_info.display_name,
      Wt::TextFormat::Plain);
		hdr->setStyleClass(actor_info.deleted ? "kb-history-header user-deleted" : "kb-history-header");
...
void task_editor_form_widget::rebuild_comments()
{
	...
	else
	{
		for(const auto& c: comments)
		{
			...
			const auto author_info = m_db.resolve_user(c.author);
			hdr2->addNew<Wt::WText>(author_info.display_name, Wt::TextFormat::Plain)
			  ->setStyleClass(author_info.deleted ? "kb-comment-author user-deleted" : "kb-comment-author");
			...
			{
				const auto editor_info = m_db.resolve_user(c.last_edited_by);
				item->addNew<Wt::WText>(
				      "Edited by " + editor_info.display_name + " at " + fmt_ts(c.last_edited_at),
				      Wt::TextFormat::Plain)
