...
{
	user_db db{":memory:"};
	db.create_user("alice", "pw1", 0, "Alice Smith");
	db.create_user("bob", "pw2", 3, "");
...
{
	user_db db{":memory:"};
	db.create_user("alice", "pw", 0, "Old Name");
	db.update_user("alice", "New Name", 7);
	auto users = db.list_users();
	REQUIRE(users.size() == 1);
